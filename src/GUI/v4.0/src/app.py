...
class App(ctk.CTk):
    ...
    def _handle_dust_frame(self, adc_values, particles_values):
        """
        Elabora il frame in arrivo dal BLE/USB.
        """
        now = time.perf_counter()
        current_global_sum = 0 
        
        # --- 1) CONTROLLO MODALITÀ: Siamo in Auto o Manual? ---
        is_auto_mode = self.advanced_tab.read_auto_var.get()
        
        if not is_auto_mode:
            # === MODALITÀ MANUALE ===
            # Il firmware invia 32 valori, ma SOLO quello all'indice del canale
            # selezionato è in fase di aggiornamento. Gli altri sono congelati.
            # Dobbiamo pescare UN SOLO dato corretto.
            ...
            if 0 <= active_ch_idx < min(DUST_CHANNELS, len(adc_values)):
                ...
                # Buffer finestra mobile per logica salto
                ...
                # Aggiorna lo stato per il grafico
                ...
                # Logica istogramma solo sul canale attivo
                if new_events > 0:
                ...
                # ACCODIAMO 1 SOLO PUNTO (quello vero!) ALLA STORIA DEL GRAFICO
                self.channel_history[active_ch_idx].append(current_adc)
                
            self.global_count = current_global_sum
            
            # 2) Disegno a schermo (Throttling) per il Manuale
            if now - self._last_draw_time >= self._min_draw_interval:
                ...
                # Manda il comando di disegno grafico solo per il canale sotto osservazione
            ...
            return  # Esce per non eseguire il ciclo Automatico
    ...
    # ---------- CALIBRAZIONE MATERIALE ----------
    def set_material_calibration(self, a: float, b: float):
        """Passa i parametri della power law all'istogramma."""
        try:
