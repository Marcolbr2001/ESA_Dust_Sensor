...
typedef struct{
  ...
  uint16_t  MydataCharHdle;                  /**< MYDATA Characteristic Handle */
  uint16_t  RecdataCharHdle;                  /**< RECDATA Characteristic Handle */
...
/*
 ...
 00C10000000111E1AC360002A5D5C51B: Characteristic 128bits UUID
 */
...
#define COPY_MYDATA_UUID(uuid_struct)       COPY_UUID_128(uuid_struct,0x00,0xc0,0x00,0x00,0x00,0x01,0x11,0xe1,0xac,0x36,0x00,0x02,0xa5,0xd5,0xc5,0x1b)
#define COPY_RECDATA_UUID(uuid_struct)       COPY_UUID_128(uuid_struct,0x00,0xc1,0x00,0x00,0x00,0x01,0x11,0xe1,0xac,0x36,0x00,0x02,0xa5,0xd5,0xc5,0x1b)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch(p_blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
        {
          ...
          if(p_attribute_modified->Attr_Handle == (BLE_SENSOR_Context.MydataCharHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch(p_attribute_modified->Attr_Data[0])
            {
              ...
              case (0x00):
                ...
                notification.EvtOpcode = BLE_SENSOR_MYDATA_NOTIFY_DISABLED_EVT;
              ...
              case GATT_CHAR_UPDATE_SEND_NOTIFICATION:
                ...
                notification.EvtOpcode = BLE_SENSOR_MYDATA_NOTIFY_ENABLED_EVT;
                BLE_SENSOR_Notification(&notification);
          ...
          else if(p_attribute_modified->Attr_Handle == (BLE_SENSOR_Context.RecdataCharHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
            ...
            switch(p_attribute_modified->Attr_Data[0])
            {
              ...
              case (0x00):
                ...
                notification.EvtOpcode = BLE_SENSOR_RECDATA_NOTIFY_DISABLED_EVT;
                BLE_SENSOR_Notification(&notification);
              ...
              case GATT_CHAR_UPDATE_SEND_NOTIFICATION:
                ...
                notification.EvtOpcode = BLE_SENSOR_RECDATA_NOTIFY_ENABLED_EVT;
                BLE_SENSOR_Notification(&notification);
          ...
          else if(p_attribute_modified->Attr_Handle == (BLE_SENSOR_Context.RecdataCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;

            notification.EvtOpcode = BLE_SENSOR_RECDATA_WRITE_NO_RESP_EVT;
...
void BLE_SENSOR_Init(void)
{
   ...
   *                                2 for MYDATA +
   *                                2 for RECDATA +
   *                                1 for MYDATA configuration descriptor +
   *                                1 for RECDATA configuration descriptor +
   *                              = 7
   ...
   * MYDATA
   */
  COPY_MYDATA_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(BLE_SENSOR_Context.Ble_sensorSvcHdle,
                          ...
                          &(BLE_SENSOR_Context.MydataCharHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("  Fail   : aci_gatt_add_char command   : MYDATA, error code: 0x%2X\n", ret);
  }
  else
  {
    LOG_INFO_APP("  Success: aci_gatt_add_char command   : MydataCharHdle = 0x%04X\n",BLE_SENSOR_Context.MydataCharHdle);
  }
   ...
   * RECDATA
   */
  COPY_RECDATA_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(BLE_SENSOR_Context.Ble_sensorSvcHdle,
                          UUID_TYPE_128,
                          (Char_UUID_t *) &uuid,
                          SizeRecdata,
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("  Fail   : aci_gatt_add_char command   : RECDATA, error code: 0x%2X\n", ret);
  }
  else
  {
    LOG_INFO_APP("  Success: aci_gatt_add_char command   : RecdataCharHdle = 0x%04X\n",BLE_SENSOR_Context.RecdataCharHdle);
  }
...
tBleStatus BLE_SENSOR_UpdateValue(BLE_SENSOR_CharOpcode_t CharOpcode, BLE_SENSOR_Data_t *pData)
{
  ...
  switch(CharOpcode)
  {
    case BLE_SENSOR_MYDATA:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        LOG_DEBUG_APP("  Fail   : aci_gatt_update_char_value MYDATA command, error code: 0x%2X\n", ret);
      }
      else
      {
        LOG_DEBUG_APP("  Success: aci_gatt_update_char_value MYDATA command\n");
      }
    ...
    case BLE_SENSOR_RECDATA:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        LOG_DEBUG_APP("  Fail   : aci_gatt_update_char_value RECDATA command, error code: 0x%2X\n", ret);
      }
      else
      {
        LOG_DEBUG_APP("  Success: aci_gatt_update_char_value RECDATA command\n");
      }
