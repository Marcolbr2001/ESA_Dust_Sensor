...
class ParticleHistogram(ctk.CTkFrame):
    ...
    def redraw(self):
        ...
        for val_adc in labels_adc:
            ...
            val_um = val_adc * LSB_TO_UM
            text_str = f"{val_um:.0f}µm"
...
class VisualTab(ctk.CTkFrame):
    ...
    def __init__(self, master, controller):
        ...
        bottom_frame.grid_rowconfigure(0, weight=0) # Titolo
        bottom_frame.grid_rowconfigure(1, weight=1) # Graph
        bottom_frame.grid_columnconfigure(0, weight=1)
        
        histo_title = ctk.CTkLabel(bottom_frame, text="Pulse Height Distribution (Size)", font=ctk.CTkFont(size=12, weight="bold"))
        histo_title.grid(row=0, column=0, sticky="nw", padx=10, pady=(5,0))
