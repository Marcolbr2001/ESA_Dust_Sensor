...
class DetailWindow(ctk.CTkToplevel):
    def __init__(self, parent, channel_id, times, values, display_mode="bit"):
        ...
        # ---------------------------------------------------------
        # NUOVA LOGICA: PALLINO E TOOLTIP (Annotazione)
        # ---------------------------------------------------------
        # Pallino che si illuminerà sul dato (stesso colore della linea ma più grande)
        self.hover_point, = self.ax.plot([], [], 'o', color=style["line"], markersize=8, 
                                         markeredgecolor=style["axes_bg"], markeredgewidth=1.5, zorder=6)
        self.hover_point.set_visible(False)

        # Fumetto
        self.annot = self.ax.annotate("", xy=(0,0), xytext=(15, 15), textcoords="offset points",
                                      bbox=dict(boxstyle="round4,pad=0.5", fc=style["fig_bg"], ec=style["line"], lw=1.5, alpha=0.9),
                                      color=style["text"], fontsize=10, zorder=5)
        ...
        # ASSE X: Testo visibile ogni 50, righe della griglia ogni 10
        self.ax.xaxis.set_major_locator(MultipleLocator(50))
        self.ax.xaxis.set_minor_locator(MultipleLocator(10))
        
        # ASSE Y: Testo visibile ogni 50 (bit) o 0.5 (voltage), griglia ogni 10 (bit) o 0.1 (voltage)
        if display_mode == "bit":
        ...
        # Disegna la griglia per i label (Major) e per i tick intermedi (Minor)
        self.ax.grid(True, which='major', linestyle='-', alpha=0.4, color=style["grid"])
        self.ax.grid(True, which='minor', linestyle=':', alpha=0.2, color=style["grid"])
        
        self.canvas = FigureCanvasTkAgg(self.fig, master=self.plot_frame)
        self.canvas.draw()
        self.canvas.get_tk_widget().pack(fill="both", expand=True)

        # Connessione Evento Mouse
    ...
    def hover(self, event):
        ...
        if event.inaxes == self.ax:
            # Controlla se il mouse è vicino alla linea del grafico (tolleranza automatica)
            cont, ind = self.line.contains(event)
            if cont:
                # Prende l'indice del punto dati più vicino al cursore
                idx = ind["ind"][0]
                x_val = self.x_indices[idx]
                y_val = self.values[idx]

                # Posiziona l'annotazione e il pallino esattamente sul punto
                self.annot.xy = (x_val, y_val)
                self.hover_point.set_data([x_val], [y_val])

                # Formatta il testo in base al display mode e all'orario
                t_val = self.times[idx]
                if not isinstance(t_val, (int, np.integer, float, np.floating)):
                    t_val = t_val.strftime('%H:%M:%S')
                ...
                if vis:
                    # Nascondi entrambi se ci si allontana dalla curva
                    self.annot.set_visible(False)
                    self.hover_point.set_visible(False)
                    self.canvas.draw_idle()
